...
export async function GET(request: NextRequest) {
  try {
    ...
    // Ensure this is a delivery agent session
    if (validation.user?.role !== 'delivery') {
      return NextResponse.json(
        { isValid: false, error: 'Not a delivery agent session' },
        { status: 403 }
      );
    }

    console.log('✅ Delivery agent session validated successfully for:', validation.user.email);
    ...
    console.error('Delivery agent session validation error:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Delivery agent session action error:', error);
    return NextResponse.json(
